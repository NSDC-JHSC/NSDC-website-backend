...
const register = async (req, res) => {
  ...
  if (existingUserName) {
    return res
      .status(409)
      .json({ error: "Username already in use", success: false });
  }
  ...
  const emailToken = signEmailVerificationToken({
...
const refresh = async (req, res) => {
  ...
  if (!refreshToken) {
    ...
    return res
      .status(400)
      .json({ error: "Missing refresh token", success: false });
  }
  ...
  try {
    ...
    return res
      .status(401)
      .json({ error: "Invalid or expired refresh token", success: false });
  }
  ...
  if (!user || user.refreshToken !== refreshToken) {
    ...
    return res
      .status(401)
      .json({ error: "Refresh token not recognized", success: false });
  }
